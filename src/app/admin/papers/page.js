...
export default function AdminPapers() {
  ...
  const fetchPapers = async () => {
    setLoading(true);
    try {
      const res = await api.get("/papers?per_page=100");
  ...
  return (
    <AdminLayout>
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-black text-4xl uppercase mb-1">Papers</h1>
          <p className="font-mono text-[var(--color-text-secondary)] text-sm font-bold uppercase tracking-widest">
            Manage Research Reading List
          </p>
        </div>
        <Button href="/admin/papers/new" variant="primary" icon={<FiPlus />}>
          New Paper
        </Button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left font-mono text-sm">
            <thead className="bg-[var(--color-bg-surface)] text-[var(--color-text-secondary)] uppercase tracking-wider border-b border-[var(--color-border-subtle)] text-xs font-semibold">
              <tr>
                <th className="p-4 border-none">Paper Title</th>
                <th className="p-4 border-none hidden md:table-cell">Authors</th>
                <th className="p-4 border-none">Discussed</th>
            ...
            <tbody className="divide-y divide-[var(--color-border-subtle)]">
              {papers.length > 0 ? (
                papers.map((paper) => (
                  <tr key={paper.id} className="hover:bg-[var(--color-bg-surface)] transition-colors">
                    <td className="p-4 border-none">
                      <div className="font-display font-bold uppercase text-base">{paper.title}</div>
                      <div className="text-[var(--color-text-secondary)] truncate max-w-[200px]">{paper.slug}</div>
                    </td>
