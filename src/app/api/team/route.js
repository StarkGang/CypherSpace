...
export async function GET(request) {
  try {
    ...
    const total = await db.collection('team').countDocuments(query);
    const cursor = db.collection('team').find(query);
...
export async function POST(request) {
  try {
    ...
    return successResponse(entityToDict(doc), 'Tea created', 201);
  } catch (error) {
    console.error(error);
    return errorResponse('Internal Server Error', 500);
  }
