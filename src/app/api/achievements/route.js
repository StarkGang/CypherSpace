...
export async function GET(request) {
  try {
    ...
    const total = await db.collection('achievements').countDocuments(query);
    const cursor = db.collection('achievements').find(query);
...
export async function POST(request) {
  try {
    ...
    return successResponse(entityToDict(doc), 'Achievement created', 201);
  } catch (error) {
    console.error(error);
    return errorResponse('Internal Server Error', 500);
  }
