...
export async function GET(request, props) {
  ...
  try {
    ...
    try {
      item = await db.collection('achievements').findOne({ _id: new ObjectId(identifier) });
    } catch (e) {}

    if (!item && 'achievements' !== 'team') {
      item = await db.collection('achievements').findOne({ slug: identifier });
    }
...
export async function PUT(request, props) {
  ...
  try {
    ...
    const db = await getDb();
    const existing = await db.collection('achievements').findOne({ _id: new ObjectId(params.id) });
    ...
    if ('achievements' !== 'team' && data.title && data.title !== existing.title) {
    ...
    const updated = await db.collection('achievements').findOne({ _id: new ObjectId(params.id) });
    
    revalidateTag('achievements');
    revalidateTag('homepage');
    revalidateTag('activity');

    return successResponse(entityToDict(updated), 'Achievement updated');
  } catch (error) {
    console.error(error);
    return errorResponse('Internal Server Error', 500);
  }
...
export async function DELETE(request, props) {
  ...
  try {
    ...
    await db.collection('activity').deleteMany({ entity_type: 'achievement', entity_id: params.id });
    
    revalidateTag('achievements');
    revalidateTag('homepage');
    revalidateTag('activity');

    return successResponse(null, 'Achievement deleted');
  } catch (error) {
    console.error(error);
    return errorResponse('Internal Server Error', 500);
  }
