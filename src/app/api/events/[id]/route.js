...
export async function GET(request, props) {
  ...
  try {
    ...
    try {
      item = await db.collection('events').findOne({ _id: new ObjectId(identifier) });
    } catch (e) {}

    if (!item && 'events' !== 'team') {
      item = await db.collection('events').findOne({ slug: identifier });
    }
...
export async function PUT(request, props) {
  ...
  try {
    ...
    const db = await getDb();
    const existing = await db.collection('events').findOne({ _id: new ObjectId(params.id) });
    ...
    if ('events' !== 'team' && data.title && data.title !== existing.title) {
    ...
    const updated = await db.collection('events').findOne({ _id: new ObjectId(params.id) });
    
    revalidateTag('events');
    revalidateTag('homepage');
    revalidateTag('activity');

    return successResponse(entityToDict(updated), 'Event updated');
  } catch (error) {
    console.error(error);
    return errorResponse('Internal Server Error', 500);
  }
...
export async function DELETE(request, props) {
  ...
  try {
    ...
    await db.collection('activity').deleteMany({ entity_type: 'event', entity_id: params.id });
